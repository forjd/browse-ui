...
Treat visual phrasing as an explicit screenshot request. If the user says things like "show me", "let me see", "what does it look like", "take a look", or "visual check", you must capture at least one `browse screenshot` after the relevant page or UI state loads.

For these requests, a text-only `browse snapshot` is not enough on its own. Finish with a screenshot-producing step unless the page failed to load or the browser command itself failed.
